%% Example 1a
% |*Goal:* Manually build a geometry, to be later encapsulated in a geometry 
% template.|
% 
% |*Result:* A valid geometry visualized and meshed.|
% 
% This script demonstrates building a geometry from scratch, using a simple 
% industrial-style straight-magnet IPM as an example. 
% 
% The geometry is built inside a script rather than a function, to maintain 
% access to all variables for easier debugging. At the same time, the code is 
% organized in such a way that it could be reused inside a |.create_geometry| 
% method of a new template, with minimal changes.
% 
% Please also see the <https://www.emdtool.com/documentation/templates/geometry_creation.html 
% second, template-creation tutorial found here>.
%% Foundations
% Dimensions
% Aligned with the template philosophy, we first set up a structure of all the 
% dimensions required. Normally, these would of course set up outside the template 
% class-to-be, but keeping them in the same script here keeps things neat and 
% tidy.
...
% Building Blocks
% Here, we begin actually creating the geometry. As the very first step, we 
% instantiate an empty <https://www.emdtool.com/api/SynRotorBase.html |SynRotorBase|> 
% object, to have access to some usable methods.
...
% 
% Next, we set up three building blocks that recur in every template (with the 
% possible exception of circuits).
...
%% 
% We use the |.create_and_add_material| utility method which achieves multiple 
% goals at once:
%% 
% * Avoids adding multiple copies of the same material (if e.g. shaft and core 
% material turned out to be the same)
% * Creates a material object from an integer, if <https://www.emdtool.com/api/get_defaultMaterials.html 
% some built-in common materials are used>.
% * *Creates a new copy of an given material object.* This is important from 
% statelessness / statefulness point of view, as e.g. |mmag != dim.magnet_material|. 
% On the flip side, clear copies generally _are_ needed; having a same material 
% object for both the stator and rotor both would result in element indexing errors.
% Domain Initialization
% We create <https://www.emdtool.com/api/Domain.html Domains> for representing 
% all the important parts of the geometry, and add them to the object.
...
% Creating Circuits
% This is not strictly needed, but in general we need some Circuits - an |EMDtool| 
% term for anything carrying a current density, be it windings or induced macroscale 
% eddy currents.
% 
% Let's add a circuit for the magnets. We also include a check for the conductivity, 
% to avoid simulation errors from an infinite resistivity.
...
%% 
% * <https://www.emdtool.com/api/Point.html |Point|> objects represent points
% * <https://www.emdtool.com/api/Curve.html |Curves|> (<https://www.emdtool.com/api/Line.html 
% |Lines|> and <https://www.emdtool.com/api/Arc.html |Arcs|>) join points
% * A set of Curves defines a <https://www.emdtool.com/api/Surface.html |Surface|>
% * A Surface (or multiple) are assigned to a Domain
%% 
% There are some utility methods for all, such as mirroring and translations. 
% In general these all *return a copy* of the original object. If this is undesired, 
% most methods have an |_inplace| version of the same method, acting on the original 
% object.
% 
% Curves are created automatically when calling the |surface.add_curve| method. 
% However, we can also create them manually if needed. Both are demonstrated in 
% the example.
% 
...
%% 
% Duplicate points _should_ be handled at the meshing stage, and are sometimes 
% unavoidable (e.g. when using Surface copying methods).
% 
% Furthermore, there aren't any automatic validity checks for the geometry, 
% so care should be taken that
%% 
% * Curves do not cross. If they do, a new Point should be defined for the crossing 
% point, and the Curves split accordingly.
% * Surfaces don't overlap.
% Utility Numbers
% First, we calculate some utility numbers. We set the _characteristic lengths_ 
% - target mesh edge lengths in any given part of the geometry. Furthermore, we 
% calculate the pitch of the pole in radians.
...
% Actual Geometry
% First, we create a surface for the magnet. We will have the magnet oriented 
% along the x-axis in the beginning, and then rotate it for the standard the EMDtool 
% orientation.
...
%% 
% At this point, we visualize the magnet to make certain all is as we expect. 
% Obviously, these visualization lines would be omitted in an actual template.
...
score.plot('r');
%% 
% Next, we tell EMDtool that the magnet is *inside* the core, called a hole. 
% Note that the magnet must be *fully inside* the core. If it breaks the outer 
% surface, it is not a hole but a separate surface, in which case the core surface 
% must be redefined accordingly.

%adding magnet as a hole
score.add_hole(smag);
%% 
% Finally, we create some air pockets at the short edges of the magnet. These 
% consist of two straight line segments (one being the short edge of the magnet), 
% and an Arc leaving the desired thickness of iron between the air pocket and 
% the rotor OD.
...
%% 
% Finally, we finalize the geometry, by adding the Surfaces to the corresponding 
% Domains.
...
%% Geometry is ready
% This is where we would return from a template's |.create_geometry| method. 
% We check the feasibility of the geometry - this method checks for overlapping 
% Curves and Surfaces. Not foolproof and misses T-intersections, but a nice additional 
% step, still.

rotor = this;
rotor.check_feasibility(); %should return 1 or true
%% 
% Finally, we manually mesh the geometry and visualize both the mesh and the 
% geometry.
